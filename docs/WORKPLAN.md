...
| C.13 | `ledger` domain (DONE, brought forward into C.10 by backend-porter, see CONTEXT.md) + **Pack 3** (`forecast.py`, `calibration.py`, `conformal.py`, `drift.py`, `montecarlo.py`, `risk.py`) | statistician | C.10 | DONE, all 21 Pack-3 services real, 194 known-answer tests. MASE gate verified both ways (Holt-Winters 0.156 vs naive 1.001; loses 1.251 vs 1.002 on a seasonal random walk and the baseline is substituted). Conformal coverage 0.898/0.901 vs nominal 0.90 and 0.949/0.951 vs 0.95, inside the theorem band. Naive resolved-only ETA covers 76% where it claims 90%. Seven catalog known-answer corrections; see CONTEXT.md |
...
| C.19 | Seed script: two demo tenants (`vaikunth-heights`/`rwa_society`, `aavartan-robotics`/`campus_club`) with enough history for the packs to be non-trivial | backend-porter | C.15 | DONE, `backend/scripts/seed_demo.py`, idempotent by full rebuild. 12/81 and 11/81 services materialize a real reading (the rest need streams the current materializer does not wire yet, e.g. `ledger`/`participation`/`decision` units, or take extra required params); the Condorcet cycle, STV election and MES budget allocation were independently verified by calling `app/stats/voting.py`/`budgeting.py` directly against the seeded ballots, not just asserted. See CONTEXT.md |
| C.20 | Deploy: `web` + `worker` Dockerfiles, `docker-compose.yml`, `.env.example` | backend-porter | C.19 | DONE, `backend/Dockerfile.web`/`Dockerfile.worker`, `docker-compose.yml`, `docs/RUNNING_LOCALLY.md`. Docker itself is not available in this sandbox so `docker compose up` could not be run end to end; every piece was tested individually against the real Postgres instead (migrations, seed, entrypoint scripts, the materializer worker script's own bug found and fixed). Vercel config not added, no `frontend/vercel.json` existed to extend and none was requested beyond `.env.example`; Neon migration not attempted, no real Neon credentials available in this sandbox. See CONTEXT.md |
