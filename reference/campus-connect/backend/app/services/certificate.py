...
from app.repository import CertificateRepository, StudentRepository, NotificationRepository
...
class CertificateService:
    def __init__(self, certificate_repo: CertificateRepository, student_repo: StudentRepository,
    ...
    async def issue(self, registration_id: int) -> Certificate | None:
        row = await self.certificate_repo.issue_context(registration_id)
        if row is None:
            return None
        registration, event, club, college, student_name, signatory_name = row
    ...
    async def my_certificates(self, payload: dict) -> list[MyCertificateItem]:
        ...
        rows = await self.certificate_repo.list_by_student(student.id)
        return [
            MyCertificateItem(
    ...
    async def verify(self, serial: str) -> CertificateVerification:
        _, event, club, college, student_name, _, certificate = await self._by_serial(serial)
...
async def issue_certificate_job(registration_id: int) -> None:
    async with SessionLocal() as db:
        service = CertificateService(
            CertificateRepository(db), StudentRepository(db), NotificationRepository(db), storage
        )
        try:
            await service.issue(registration_id)
            await db.commit()
        except Exception:
            await db.rollback()
