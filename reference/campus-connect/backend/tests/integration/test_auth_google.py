...
@pytest.mark.asyncio
async def test_google_signup_creates_student_for_matching_college_domain(client, seed_college, mock_google_token):
    """Verify that Google signup with matching college domain creates student"""
    mock_google_token(google_claims("newstudent@knit.edu.in"))
...
@pytest.mark.asyncio
async def test_google_login_existing_user_succeeds(client, seed_college, mock_google_token):
...
@pytest.mark.asyncio
async def test_google_login_links_existing_password_account(client, seed_college, mock_google_token):
...
# @pytest.mark.asyncio
# async def test_google_signup_without_matching_college_creates_campus_admin(client, mock_google_token):
...
@pytest.mark.asyncio
async def test_google_login_with_no_matching_college_fails(client, mock_google_token):
    ...
    assert response.json()["message"] == "College not registered"


@pytest.mark.asyncio
async def test_google_login_with_matching_college_but_no_account_fails(client, seed_college, mock_google_token):
...
@pytest.mark.asyncio
async def test_google_auth_unverified_email_fails(client, seed_college, mock_google_token):
...
@pytest.mark.asyncio
async def test_google_signup_subdomain_of_college_matches(client, seed_college, mock_google_token):
    """Verify that college subdomains are matched correctly"""
    mock_google_token(google_claims("newstudent@cs.knit.edu.in"))
...
@pytest.mark.asyncio
async def test_google_auth_email_case_insensitive_match(client, seed_college, mock_google_token):
...
@pytest.mark.asyncio
async def test_google_signup_twice_with_same_email_treats_second_as_login(client, seed_college, mock_google_token):
