...
@pytest.mark.asyncio
async def test_list_notifications_empty_for_new_student(client, outsider):
    """Verify that a student with no notifications gets an empty list"""
...
@pytest.mark.asyncio
async def test_unread_count_zero_for_new_student(client, outsider):
    """Verify that a student with no notifications has an unread count of zero"""
...
@pytest.mark.asyncio
async def test_mark_all_notifications_read_zero_when_none_unread(client, seed_college):
    """Confirm that read-all reports zero updated when there are no unread notifications"""
    payload = {
        "email": "freshstudent@knit.edu.in",
...
async def test_mark_notification_read_success(client, leader, member):
    """Verify that a student can mark their own notification as read"""
    headers, club_id = leader
    payload = {
        "club_id": club_id,
...
@pytest.mark.asyncio
async def test_mark_notification_read_belonging_to_another_student_fails(client, leader, member, outsider):
    """Ensure that a student cannot mark another student's notification as read"""
    headers, club_id = leader
    payload = {
        "club_id": club_id,
        "title": "Cross student read event",
        "description": "An event used to test cross-student notification access",
        "venue": "Lab 204",
        "starts_at": future_time(48),
        "ends_at": future_time(50),
    }
...
async def test_join_approved_creates_notification(client, leader, outsider):
    """Verify that approving a join request creates a JOIN_APPROVED notification with the club name"""
    leader_headers, club_id = leader
    join = await client.post(f"/clubs/{club_id}/join", headers=outsider)
    action_payload = {"action": "APPROVED"}
    await client.patch(
        f"/clubs/{club_id}/requests/{join.json()['id']}", headers=leader_headers, json=action_payload
    )
    ...
    assert len(body) == 1
    assert body[0]["type"] == "JOIN_APPROVED"
    assert body[0]["message"] == "Your request to join Robotics Club was approved"
    assert body[0]["club_id"] == club_id
...
async def test_join_rejected_creates_notification(client, leader, outsider):
    """Verify that rejecting a join request creates a JOIN_REJECTED notification with the club name"""
    leader_headers, club_id = leader
    join = await client.post(f"/clubs/{club_id}/join", headers=outsider)
    ...
    assert body[0]["message"] == "Your request to join Robotics Club was not approved"
...
@pytest.mark.asyncio
async def test_mark_all_notifications_read_does_not_affect_other_students(client, leader, member, outsider):
    """Verify that read-all for one student does not mark another student's notifications as read"""
    leader_headers, club_id = leader
    join = await client.post(f"/clubs/{club_id}/join", headers=outsider)
    action_payload = {"action": "APPROVED"}
    await client.patch(
...
async def test_registration_failure_does_not_create_notification(client, leader, member, outsider):
    ...
    await client.patch(
        f"/clubs/{club_id}/requests/{join.json()['id']}", headers=leader_headers, json=approve_payload
    )
...
@pytest.mark.asyncio
async def test_list_notifications_never_returns_another_students_notifications(client, leader, member, outsider):
    """Verify that a student's notification list never includes another student's notifications"""
    leader_headers, club_id = leader
    join = await client.post(f"/clubs/{club_id}/join", headers=outsider)
    approve_payload = {"action": "APPROVED"}
    await client.patch(
