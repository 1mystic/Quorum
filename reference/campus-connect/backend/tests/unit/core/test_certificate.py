...
def test_render_pdf_all_result_templates(result):
    ...
    context = CertificateContext(
        ...
        signatory_role="Club President",
    )
    pdf_bytes = render_pdf(context)
    assert pdf_bytes[:4] == b"%PDF"


def test_render_pdf_handles_long_student_name():
    """Confirm that a very long student name doesn't crash PDF rendering"""
    context = CertificateContext(
        serial="CC-TEST-2026-00001",
...
@pytest.mark.asyncio
async def test_issue_skips_non_checked_in_registration():
    """Confirm that issue() returns None for a registration that was never checked in"""
    ...
    certificate_repo.issue_context.return_value = (
        registration, event, club, college, "Test Student", "Test Signatory"
    )
    ...
    result = await service.issue(registration_id=1)
    assert result is None


@pytest.mark.asyncio
async def test_issue_skips_registrant_result():
    """Confirm that issue() returns None when checked in but result is still REGISTRANT"""
...
@pytest.mark.asyncio
async def test_issue_skips_missing_registration():
    """Confirm that issue() returns None when the registration ID doesn't exist"""
    certificate_repo = AsyncMock()
    ...
    result = await service.issue(registration_id=99999)
    assert result is None
