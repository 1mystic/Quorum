...
from app.repository import UserRepository, TenantRepository
from app.exceptions import (
    UserAlreadyExistError, TenantNotFoundError, TenantAlreadyExistError,
...
class UserService:
    ...
    async def signup(self, data: SignupRequest):
        ...
        elif data.role == UserRole.TENANT_ADMIN:
            new_tenant_admin = await self.user_repo.create_tenant_admin(user_id=new_user.id)
    ...
    async def login(self, data: LoginRequest):
        ...
        slug = await self.tenant_repo.id_to_slug(user.tenant_id) if user.tenant_id else None
    ...
    async def google_auth(self, data: GoogleAuthRequest) -> GoogleAuthResponse:
        ...
        if user:
            ...
            tenant = await self.tenant_repo.get_by_slug(data.tenant_slug) if data.tenant_slug else None
            if data.tenant_slug and not tenant:
                raise TenantNotFoundError()
            role = UserRole.MEMBER if tenant else UserRole.TENANT_ADMIN
            tenant_id = tenant.id if tenant else None
