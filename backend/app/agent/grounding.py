...
allow-list that never gives the model a group it may not name cannot.

    Gate 1  Tool-return allow-list   (here)
    Gate 2  Tenant scoping          (every service call takes payload, which
                                      resolves the caller's own tenant - a
                                      tool has no tenant_id parameter to abuse)
    ...
    Gate 5  Read-only tool surface   (tools.py calls only .list()/.get()/
                                      .my_groups()/.feed() - no write method
...
class AllowList:
    ...
    # (kind, id) -> display name, e.g. ("group", 3) -> "Robotics & Automation"
...
def apply_output_gates(text: str, allow_list: AllowList) -> tuple:
    ...
    # prose, which would leak the internal listing format to the member.
...
def redact_for_model(rows: list, allowed_fields: list) -> list:
    ...
    The upstream API already omits emails from group and event payloads, but
