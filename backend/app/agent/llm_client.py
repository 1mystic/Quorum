"""
Haiku client for the AI Group Finder (design doc Section 4).
...
it never decides which groups or events exist.
...
# ---------------------------------------------------------------------------
_CANDIDATE_RE = re.compile(r"group_id=(\d+) \| ([^|]+) \| ([^|]+) \|")
_AVAILABLE_ENTITY_RE = re.compile(r"\[\[(group|event):(\d+)\]\] ([^(\-]+)")


def _mock_finder(prompt: str) -> str:
    reasons = [
        {"group_id": int(group_id), "reason": f"Matches your interests and fits {category.strip().lower()} activities."}
        for group_id, name, category in _CANDIDATE_RE.findall(prompt)
    ]
...
def _mock_chat(system_prompt: str) -> str:
    ...
    if not match:
        return "I could not find a group or public event that matches yet - try describing a different interest."
