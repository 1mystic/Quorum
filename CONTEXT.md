...
- **statistician**: **A.2 to A.8, C.6, C.7 and C.9 complete.** The statistical architecture exists in
  ...
  **Next statistician card is C.13** (`ledger` domain plus Pack 3), which needs the ledger model
  first. `streams/reduce.py` and `streams/capacity.py` are the other open statistician items: the
  backend now has a `RequestEventLog` table, so `reduce.request_spells` can be written against real
  atoms rather than against a shape
